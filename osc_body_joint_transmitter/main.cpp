...
void PrintUsage()
{
    printf("\nUSAGE: (k4abt_)osc_body_joint_transmitter.exe SensorMode[NFOV_UNBINNED, WFOV_BINNED](optional) RuntimeMode[CPU](optional)\n");
    printf("  - SensorMode: \n");
    ...
    printf("e.g.   (k4abt_)osc_body_joint_transmitter.exe WFOV_BINNED CPU\n");
    printf("e.g.   (k4abt_)osc_body_joint_transmitter.exe CPU\n");
    printf("e.g.   (k4abt_)osc_body_joint_transmitter.exe WFOV_BINNED\n");
}
...
void PrintAppUsage()
{
	...
	printf(" c: send calibration\n");
	printf(" s: stream osc messages\n");
	printf("\n");
}
...
int main(int argc, char** argv)
{
	...
	UdpTransmitSocket transmitSocket(IpEndpointName("127.0.0.1", 57120));
    ...
    while (s_isRunning)
    {
        ...
        if (popFrameResult == K4A_WAIT_RESULT_SUCCEEDED)
        {
            ...
            for (size_t i = 0; i < numBodies; i++)
            {
				...
				//if (body.skeleton.joints[K4ABT_JOINT_SPINE_CHEST].confidence_level >= K4ABT_JOINT_CONFIDENCE_LOW)
				if (send_data == true)
				{
					char buffer[OUTPUT_BUFFER_SIZE];
					osc::OutboundPacketStream p(buffer, OUTPUT_BUFFER_SIZE);
					...
					p << osc::BeginBundleImmediate
						<< osc::BeginMessage("/spineChest")
						<< body.skeleton.joints[K4ABT_JOINT_SPINE_CHEST].position.xyz.x
						<< body.skeleton.joints[K4ABT_JOINT_SPINE_CHEST].position.xyz.y
						<< body.skeleton.joints[K4ABT_JOINT_SPINE_CHEST].position.xyz.z << osc::EndMessage;
				...
				if (send_data == true)
				if (body.skeleton.joints[K4ABT_JOINT_WRIST_RIGHT].confidence_level >= K4ABT_JOINT_CONFIDENCE_LOW)
				{
					char buffer[OUTPUT_BUFFER_SIZE];
					osc::OutboundPacketStream p(buffer, OUTPUT_BUFFER_SIZE);

					p << osc::BeginBundleImmediate
						<< osc::BeginMessage("/rightWrist")
						<< body.skeleton.joints[K4ABT_JOINT_WRIST_RIGHT].position.xyz.x
						<< body.skeleton.joints[K4ABT_JOINT_WRIST_RIGHT].position.xyz.y
						<< body.skeleton.joints[K4ABT_JOINT_WRIST_RIGHT].position.xyz.z << osc::EndMessage;

					transmitSocket.Send(p.Data(), p.Size());
				}

				if (send_data == true)
				if (body.skeleton.joints[K4ABT_JOINT_WRIST_LEFT].confidence_level >= K4ABT_JOINT_CONFIDENCE_LOW)
				{
					char buffer[OUTPUT_BUFFER_SIZE];
					osc::OutboundPacketStream p(buffer, OUTPUT_BUFFER_SIZE);

					p << osc::BeginBundleImmediate
						<< osc::BeginMessage("/leftWrist")
						<< body.skeleton.joints[K4ABT_JOINT_WRIST_LEFT].position.xyz.x
						<< body.skeleton.joints[K4ABT_JOINT_WRIST_LEFT].position.xyz.y
						<< body.skeleton.joints[K4ABT_JOINT_WRIST_LEFT].position.xyz.z << osc::EndMessage;

					transmitSocket.Send(p.Data(), p.Size());
				}

				if (send_calibration == true) {
					if (body.skeleton.joints[K4ABT_JOINT_WRIST_RIGHT].confidence_level >= K4ABT_JOINT_CONFIDENCE_LOW)
					{
						char buffer[OUTPUT_BUFFER_SIZE];
						osc::OutboundPacketStream p(buffer, OUTPUT_BUFFER_SIZE);
						...
						p << osc::BeginBundleImmediate
							<< osc::BeginMessage("/rightWristCalib")
							<< body.skeleton.joints[K4ABT_JOINT_WRIST_RIGHT].position.xyz.x
							<< body.skeleton.joints[K4ABT_JOINT_WRIST_RIGHT].position.xyz.y
							<< body.skeleton.joints[K4ABT_JOINT_WRIST_RIGHT].position.xyz.z << osc::EndMessage;
					...
					if (body.skeleton.joints[K4ABT_JOINT_WRIST_LEFT].confidence_level >= K4ABT_JOINT_CONFIDENCE_LOW)
					{
						...
						p << osc::BeginBundleImmediate
							<< osc::BeginMessage("/leftWristCalib")
							<< body.skeleton.joints[K4ABT_JOINT_WRIST_LEFT].position.xyz.x
							<< body.skeleton.joints[K4ABT_JOINT_WRIST_LEFT].position.xyz.y
							<< body.skeleton.joints[K4ABT_JOINT_WRIST_LEFT].position.xyz.z << osc::EndMessage;
